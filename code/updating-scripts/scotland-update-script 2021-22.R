...
if (add.new.data){
  ...
  ################################################################################
    ...
    # title as it will appear in the references:
  sco.aberdeen.title <- "{Aberdeen City Council Annual Accounts 2021-22}"
  
  # url of the file:
  sco.aberdeen.url <- "https://committees.aberdeencity.gov.uk/documents/s133464/2.%20Appendix%20A%20ACC%20unaudited%20Annual%20Accs%2021-22.pdf"
  
  # year published, as it will appear in the references:
  sco.aberdeen.year.published <- 2022
  
  ## replace with date of access to data:
  sco.aberdeen.date.accessed <- "26.4.2023"
  
  # How much parking Income did Aberdeen City report this year?? (£,000s)  Section 18
  sco.aberdeen.income.total <- 6280
  
  # How much parking Expenditure did Aberdeen City report this year?? (£,000s)  Section 18
  sco.aberdeen.expend.total <- 2682
  
  
  # Transport Scotland (PCN) data  ###############################################
  ################################################################################
  # title as it will appear in the references: N.B. THIS SEEMS TO BE IS A YEAR BEHIND THE ACCOUNTS!!!
  sco.pdf.title <- "Decriminalised Parking Enforcement: Local Authorites’ Income
and Expenditure: 2021 to 2022"
  
  # url of the file:
  sco.pdf.url <- "https://www.transport.gov.scot/media/52668/decriminalised-parking-enforcement-local-authorities-income-and-expenditure-2021-to-2022.pdf"
  ...
  # path and name of file where you have saved it:
  sco.pdf.file <- "data/01-raw/orig.sco-21-22-pcn.pdf"
...
if (add.new.data){  
  ...
  scotland.dpe %>% mutate(auth.name=replace(auth.name,auth.name=="Shetland","Shetland Islands"))%>% 
    mutate(auth.name=replace(auth.name,auth.name=="Comhairle nan Eilean Sar","Na h-Eileanan an Iar"))%>% 
    mutate(auth.name=replace(auth.name,auth.name=="Orkney Islands Council","Orkney Islands"))->scotland.dpe
  ...
  scotland.pcn %>% rename(any_of(c(Local.authority="Local.Authority")))->scotland.pcn
  
  #Check for empty rows
  var<-paste0("X",current.year,".",current.year-1999)
 scotland.pcn %>% filter( get(var) == "")   ->scot.pcn.empty
    if(current.year>2021 & nrow(scot.pcn.empty)!=0){stop("Blank Rows from PDF - Check Data")}
  
#Fix for 2018 Data 
 if(current.year==2021 & nrow(scot.pcn.empty)!=0){
