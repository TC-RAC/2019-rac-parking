...
if (add.new.data){
  ...
  sco.i.e.title <- "Scottish Local Government Finance Statistics 2018-19: Local Authority Level Analysis - Net Revenue Expenditure by Subservice"
  ...
  sco.i.e.year.published <- 2020
  
  ## replace with date of access to data:
  sco.i.e.date.accessed <- "20.04.2023"
  
  # path and name of file where you have saved it:
  sco.i.e.file <- "data/01-raw/orig.sco-18-19.xlsx"
  ...
  ################################################################################
    ...
    # title as it will appear in the references:
  sco.aberdeen.title <- "{Aberdeen City Council Annual Accounts 2018-19}"
  
  # url of the file:
  sco.aberdeen.url <- "https://www.aberdeencity.gov.uk/sites/default/files/2020-08/ACC%20Scanned%20signed%20audited%20Annual%20Accounts%202018-19.pdf"
  
  # year published, as it will appear in the references:
  sco.aberdeen.year.published <- 2019
  
  ## replace with date of access to data:
  sco.aberdeen.date.accessed <- "24.4.2023"
  
  # How much parking Income did Aberdeen City report this year?? (£,000s)  Section 18
  sco.aberdeen.income.total <- 7869
  
  # How much parking Expenditure did Aberdeen City report this year?? (£,000s)  Section 18
  sco.aberdeen.expend.total <- 4981
...
and Expenditure: 2018 to 2019"
  ...
  # url of the file:
  sco.pdf.url <- "https://www.transport.gov.scot/media/46552/decriminalised-parking-enforcement-income-expenditure-annual-report-2018-19.pdf"
  
  # year published, as it will appear in the references:
  sco.pdf.year.published <- 2018
  
  ## replace with date of access to data:
  sco.pdf.date.accessed <- "20.04.2023"
  
  # path and name of file where you have saved it:
  sco.pdf.file <- "data/01-raw/orig.sco-18-19-pcn.pdf"
...
if (add.new.data){  
 ...
 scotland.pcn %>% filter( get(var) == "")   ->scot.pcn.empty
    if(current.year>2018 & nrow(scot.pcn.empty)!=0){stop("Blank Rows from PDF - Check Data")}
