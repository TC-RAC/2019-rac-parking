...
if (add.new.data){
  ...
  eng.i.e.title <- "Local authority revenue expenditure and financing England: 2019-20, individual local authority data - outturn"
  ...
  eng.i.e.url <- "https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/992910/RO2_2019-20_data_by_LA.ods"
  
  # year published, as it will appear in the references:
  eng.i.e.year.published <- 2019

  ## replace with date of access to data:
  eng.i.e.date.accessed <- "11.04.2023"

  # path and name of file where you have saved it:
  eng.i.e.file <- "data/01-raw/orig.eng-19-20.xlsx"
  ...
  #############################################################################
  # title as it will appear in the references:
  eng.budg.title <- "Local authority revenue expenditure and financing England: 2020 to 2021 budget (Revenue Account budget)"
  
  # url of the file:
  eng.budg.url <- "https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/922964/RA_2020-21_data_by_LA_rev.xlsx"
  
  # year published, as it will appear in the references:
  eng.budg.year.published <- 2020
  
  ## replace with date of access to data:
  eng.budg.date.accessed <- "22.04.2023"
  
  # path and name of file where you have saved it:
  eng.budg.file <- "data/01-raw/orig.eng-20-21-budget.xlsx"
...
#master<-master[!is.na(auth.code),];saveRDS(master,"data/03-processed/master.rds")
