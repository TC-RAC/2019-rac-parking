...
if (add.new.data){
  ...
  sco.i.e.title <- "Scottish Local Government Finance Statistics 2019-20: Local Authority Level Analysis - Net Revenue Expenditure by Subservice"
  ...
  ################################################################################
    ...
    # title as it will appear in the references:
  sco.aberdeen.title <- "{Aberdeen City Council Annual Accounts 2019-20}"
  
  # url of the file:
  sco.aberdeen.url <- "https://www.aberdeencity.gov.uk/sites/default/files/2020-08/ACC%20Audited%20Annual%20Accounts%202019-20.pdf"
  
  # year published, as it will appear in the references:
  sco.aberdeen.year.published <- 2020
  
  ## replace with date of access to data:
  sco.aberdeen.date.accessed <- "26.4.2023"
  
  # How much parking Income did Aberdeen City report this year?? (£,000s)  Section 18 Trading Operations
  sco.aberdeen.income.total <- 8154
  
  # How much parking Expenditure did Aberdeen City report this year?? (£,000s)  Section 18
  sco.aberdeen.expend.total <- 4077
...
and Expenditure: 2018 to 2019"
  ...
  # url of the file:
  sco.pdf.url <- "https://www.transport.gov.scot/media/46552/decriminalised-parking-enforcement-income-expenditure-annual-report-2019-20.pdf"
  
  # year published, as it will appear in the references:
  sco.pdf.year.published <- 2021
  
  ## replace with date of access to data:
  sco.pdf.date.accessed <- "26.04.2023"
  
  # path and name of file where you have saved it:
  sco.pdf.file <- "data/01-raw/orig.sco-19-20-pcn.pdf"
...
#master %>% filter(!is.na(auth.code))->master
...
if (add.new.data){  
  ...
  #This occaisionally comes up and says "file.name" not found - it can just be run on its own after doing file.name = sco.i.e.file and it seems to go fine!
 ...
 scotland.pcn %>% filter( get(var) == "")   ->scot.pcn.empty
    if(current.year>2019 & nrow(scot.pcn.empty)!=0){stop("Blank Rows from PDF - Check Data")}
  
#Fix for 2018 Data 
 if(current.year==2018 & nrow(scot.pcn.empty)!=0){
...
#  scotland.tfs.i.e <- extract_tables(sco.pdf.file,
  scotland.tfs.i.e <- extract_areas(sco.pdf.file, 
                                     pages = sco.pdf.i.e.tab,
                                     output = "data.frame")[[1]]
  
  scotland.tfs.i.e[scotland.tfs.i.e==""]<-NA
  scotland.tfs.i.e  %>%  na.omit() -> scotland.tfs.i.e
