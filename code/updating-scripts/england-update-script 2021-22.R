...
if (add.new.data){
  ...
  #Outturn Data = Current Year to Current Year +1
  #Revenue Data = Current Year +1 to Current Year +2

  #############################################################################
  # England outturn data (Income and Expenditure data)                        #
  #############################################################################
  # title as it will appear in the references:
  eng.i.e.title <- "Local authority revenue expenditure and financing England: 2021-22, individual local authority data - outturn"
  ...
  eng.i.e.url <- "https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/1144979/RO2_2021-22_data_by_LA.ods"
  
  # year published, as it will appear in the references:
  eng.i.e.year.published <- 2022

  ## replace with date of access to data:
  eng.i.e.date.accessed <- "18.04.2023"

  # path and name of file where you have saved it:
  eng.i.e.file <- "data/01-raw/orig.eng-21-22.xlsx"

  # on which sheet is the LA data on?
  eng.i.e.sh <- 4
  
  # on which sheet is the Summary data on?
  eng.i.e.sh.summary <- 3
  ...
  #############################################################################
  # title as it will appear in the references:
  eng.budg.title <- "Local authority revenue expenditure and financing England: 2022 to 2023 budget (Revenue Account budget)"
  
  # url of the file:
  eng.budg.url <- "https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/1097173/RA_2022-23_data.ods"
  
  # year published, as it will appear in the references:
  eng.budg.year.published <- 2022
  
  ## replace with date of access to data:
  eng.budg.date.accessed <- "18.04.2023"
  
  # path and name of file where you have saved it:
  eng.budg.file <- "data/01-raw/orig.eng-22-23-budget.xlsx"
